...
class FileSystemEmailThreadCacheStrategy(CachingStrategy):
    def __init__(self, output_basedir: str, project_name: str, user_email: str):
        ...
        self.message_id_to_date: Dict[str, str] = {}
        super().__init__(output_basedir, project_name, user_email)

    def fill_cache(self):
        threads_dir = FileUtils.join_path(self.project_acct_basedir, THREADS_DIR)
        ...
        for thread_id in self.thread_ids:
            ...
            self.message_id_to_date.update(JsonFileUtils.load_data_from_json_file(message_data_file))
        LOG.debug(f"Loaded message data: {self.message_id_to_date}")

    def handle_threads(self, thread_response: Dict[str, Any], thread: Thread):
        thread_id: str = GH.get_field(thread_response, ThreadField.ID)
        threads_dir = FileUtils.ensure_dir_created(FileUtils.join_path(self.project_acct_basedir, THREADS_DIR))
        ...
        message_id_to_date: Dict[str, str] = {GH.get_field(msg, MessageField.ID): GH.get_field(msg, MessageField.DATE) for msg in messages_response}
        message_data_file = FileUtils.join_path(current_thread_dir, MESSAGE_DATA_FILENAME)
        JsonFileUtils.write_data_to_file_as_json(message_data_file, message_id_to_date, pretty=True)
